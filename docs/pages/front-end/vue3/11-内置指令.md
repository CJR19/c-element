...
- **详细信息**
  ...
  `v-text` 通过设置元素的 [textContent](https://developer.mozilla.org/en-US/docs/Web/API/Node/textContent) 属性来工作，因此它将覆盖元素中所有现有的内容。如果你需要更新 `textContent` 的部分，应该使用 `mustache interpolations `代替。
...
- **详细信息**
  ...
  当同时使用时，`v-if` 比 `v-for` 优先级更高。不推荐在一元素上同时使用这两个指令 
