...
由于本章涉及大量新语法，故在本章开头列出新语法的索引，方便在使用这些新语法时能快速查找到对应的讲解：
...
既可以通过 `` 标签引入，又可以通过 import 导入的库，称为 UMD 库。相比于 npm 包的类型声明文件，我们需要额外声明一个全局变量，为了实现这种方式，ts 提供了一个新语法 export as namespace。
...
注意，三斜线指令**必须放在文件的最顶端**，三斜线指令的前面只允许出现单行或多行注释。
...
除了这两种三斜线指令之外，还有其他的三斜线指令，比如 ///, /// 等，但它们都是废弃的语法，故这里就不介绍了，详情可见官网。
